...
struct tuple_hasher {
  ...
  {
    return hashCombine(hash(std::get<Index>(t)),
                       tuple_hasher<Tuple, Index - 1>()(t));
  }
};

template <typename... Args> size_t hash(const std::tuple<Args...>& t)
{
...
{
  return hashCombine(hash(t.first), hash(t.second));
}
...
size_t hash(const std::vector<T, Allocator>& v)
{
  ...
  for (auto const& item : v)
    value = hashCombine(value, hash(item));
  return value;
}
...
template <typename T1, typename T2> struct comparator<std::pair<T1, T2>> {
  size_t operator()(const std::pair<T1, T2>& t1,
                    const std::pair<T1, T2>& t2) const
  {
