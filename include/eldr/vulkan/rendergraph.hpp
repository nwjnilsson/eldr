...
enum class TextureUsage {
  ...
  /// @brief Specifies that this texture is a multisampling render target
  offscreen_buffer,
...
public:
  TextureResource(std::string&& name, TextureUsage usage)
    : RenderResource(name), usage_(usage)
  {
  }

  /// @brief Specifies the format of this texture that is required when the
  /// physical texture is made.
  /// @details For TextureUsage::BACK_BUFFER textures, using the swapchain image
  /// format is preferable in most cases. For TextureUsage::DEPTH_STENCIL_BUFFER
  /// textures, a VK_FORMAT_D* must be used.
  void setFormat(VkFormat format) { format_ = format; }

private:
  const TextureUsage    usage_;
  VkFormat              format_{ VK_FORMAT_UNDEFINED };
  VkSampleCountFlagBits num_samples_{ VK_SAMPLE_COUNT_1_BIT };
...
private:
  ...
  std::shared_ptr<spdlog::logger> log_{ spdlog::default_logger()->clone(
    "render-graph") };
