...
Shader::Shader(const Device& device, VkShaderStageFlagBits flags,
               const std::string& name, const std::string& file_name,
               const std::string& entry_point)
  : device_(device), name_(name), entry_point_(entry_point), flags_(flags)
{
  // TODO: move
  ...
  if (const VkResult result = vkCreateShaderModule(
      ...
      result != VK_SUCCESS) {
    ThrowVk("Failed to create shader module {}! {}, ", name_, result);
  }
}
